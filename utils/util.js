...
function getRow (rowData) {
    ...
    rowStr += rowData.map(column => {
        if (typeof column === 'object') {
            if (column.row) return `<td rowspan=${column.row}>${column.val+'\t'}</td>`
            else if (column.column) return `<td colspan=${column.column}>${column.val+'\t'}</td>`
            else return `<td>${column.value+'\t'}</td>`
...
/**
 * 导出excel表格
 * @param {*} tHead 表头 Array []
 * @param {*} tBody 内容 Array []
 * @param {*} fileName 文件名
 */
// 数据示例
// const params = {
...
//         // 需要合并表格 跨列 or 跨行 
//         // row 跨行
//         [{val: '内容一', row: 2}, '内容二', '内容三'],
//         ['内容二', '内容三']
//         // column 跨列
//         [{val: '内容一', column: 2}, '内容三'],
//         ['内容一', '内容二', '内容三']
//     ],
//     fileName: '文件名'
// }
export function table2Excel({tHead, tBody, fileName}) {
    
    var tableHead = `<thead><tr>${tHead.map(h => `<th>${h}</th>`).join('')}</tr></thead>`
    var tableBody = `<tbody>${tBody.map(row => getRow(row)).join('')}</tbody>`

    var tableString = tableHead + tableBody
 ...
 * 获取主站域名
 */
export function getMainOrigin(mode) {
    if (mode === undefined && typeof window !== 'undefined' && window.INIT_DATA && window.INIT_DATA.mode) {
        mode = window.INIT_DATA.mode;
    }
    let protocol = 'https:';
    if (typeof location !== 'undefined') {
        protocol = location.protocol;
    }
    if (/dev|test/.test(mode)) {
        return `${protocol}//m.${mode}.qlchat.com`;
    } else {
        return `${protocol}//m.qlchat.com`;
    }
}
